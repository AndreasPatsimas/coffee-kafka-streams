...
public class KafkaStreamApplication {
    ...
    public static void consume() {
        ...
        KTable<String, CoffeeInformation> coffeeMachineJoinedDeviceTable = coffeeMachineTable
                .join(coffeeMachineIdKeyDeviceTable, (coffeeMachine, device) -> CoffeeInformation.newBuilder()
                .setCoffeeMachineSerial(coffeeMachine.getSerial())
                .setDeviceSerial(device.getSerial())
                .setCustomerId(coffeeMachine.getCustomerId())
                .build());

        KTable<String,CoffeeInformation> customerIdCoffeeMachineDeviceTable = coffeeMachineJoinedDeviceTable.toStream()
                .map((k,v)-> KeyValue.pair(v.getCustomerId(),v)).toTable();

        KTable<String,Customer> customerIdCustomerTable = customerDataTable.toStream()
                .map((k,v)-> KeyValue.pair(v.getId(),v)).toTable();
        ...
        KStream<String,CoffeeInformation> updatedStream = coffeeInformation
                .map((k,v) -> KeyValue.pair(v.getCoffeeId(),v));
        ...
        KStream<String,SingleVsDouble> singleVsDoubleKStream = coffeeInformation.filter((k,v) -> v.getCoffeeDosage()<3)
                .map((k,v)->{
        ...
        KTable<String, Notification> notificationTable = coffeeMachineTable.join(coffeeMachineIdKeyDeviceTable,
                (coffeeMachine, device) -> Notification.newBuilder()
